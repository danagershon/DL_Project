...
def train_variational_auto_decoder(batch_size=32,
    ...
    model = VariationalAutoDecoder(input_dim = input_dim, latent_dim=latent_dim, 
    ...
    # Initialize a random latent vector for each sample in the training set --- TODO LEFT technically this is not a latent vector anymore. maybe we can name this differently
    if latent_initialization == "random":
        latents = torch.randn(len(train_ds), input_dim, requires_grad=True, device=device)
    elif latent_initialization == "uniform":
        latents = torch.rand(len(train_ds), input_dim, requires_grad=True, device=device)
    else:  # normal
        latents = torch.normal(0, normal_latent_initialization_variance, size=(len(train_ds), input_dim), requires_grad=True, device=device)
